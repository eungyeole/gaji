...
fn main() -> Result<()> {
    ...
    match arguments[0].as_str() {
        ...
        "operation" => {
            ...
            let state = gaji_core::operation_state(path)?;
            println!("{}", serde_json::to_string_pretty(&state)?);
        }
        "cherry-pick" => {
            let (path, revision) = two_arguments(&arguments, "cherry-pick <path> <revision>")?;
            gaji_core::cherry_pick(path, revision)?;
        }
        "rebase" => {
            let (path, upstream) = two_arguments(&arguments, "rebase <path> <upstream>")?;
            gaji_core::rebase_onto(path, upstream)?;
        }
        "continue" => gaji_core::continue_operation(one_path(&arguments))?,
        "abort" => gaji_core::abort_operation(one_path(&arguments))?,
        "resolve" => {
            let path = arguments
                .get(1)
            ...
            match arguments.get(3).map(String::as_str).unwrap_or("manual") {
                "ours" => gaji_core::choose_conflict_side(path, file, ConflictSide::Ours)?,
                "theirs" => gaji_core::choose_conflict_side(path, file, ConflictSide::Theirs)?,
                "manual" => gaji_core::mark_resolved(path, file)?,
...
fn inspect(path: &str) -> Result<()> {
    let snapshot = gaji_core::inspect_repository(path)
