...
pub fn interactive_rebase_plan(
    ...
    upstream: &str,
) -> Result<Vec<RebaseStep>, RiftError> {
...
pub fn start_interactive_rebase(
    ...
    let output = Command::new("git")
        ...
        .env("RIFT_REBASE_TODO", &files.todo)
        .env("GIT_EDITOR", "true")
        .output()?;
    ...
    if output.status.success() {
        ...
        Err(RiftError::GitFailed(
            String::from_utf8_lossy(&output.stderr).trim().to_owned(),
        ))
    }
}

fn validate_plan(root: &Path, upstream: &str, steps: &[RebaseStep]) -> Result<(), RiftError> {
...
fn resolve_commit(root: &Path, revision: &str) -> Result<String, RiftError> {
    if revision.trim().is_empty() {
        return Err(RiftError::GitFailed("upstream cannot be empty".to_owned()));
    }
    git(
        root,
...
impl SequenceEditorFiles {
    fn create(todo: &str) -> Result<Self, RiftError> {
        let nonce = TEMP_SEQUENCE.fetch_add(1, Ordering::Relaxed);
        let stem = format!("rift-rebase-{}-{nonce}", std::process::id());
        ...
        create_file(
            &editor_path,
            "@copy /Y \"%RIFT_REBASE_TODO%\" \"%~1\" >NUL\r\n",
        )?;
        #[cfg(not(windows))]
        {
            create_file(&editor_path, "#!/bin/sh\ncp \"$RIFT_REBASE_TODO\" \"$1\"\n")?;
...
fn create_file(path: &Path, contents: &str) -> Result<(), RiftError> {
