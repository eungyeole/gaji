...
pub fn inspect_repository(path: impl AsRef<Path>) -> Result<RepositorySnapshot, RiftError> {
    ...
    let log = match git(
        ...
        Ok(log) => log,
        Err(RiftError::GitFailed(_)) => String::new(),
...
pub fn operation_state(path: impl AsRef<Path>) -> Result<OperationState, RiftError> {
...
pub fn rebase_onto(path: impl AsRef<Path>, upstream: &str) -> Result<(), RiftError> {
    let root = repository_root(path.as_ref())?;
    let commit = resolve_commit(&root, upstream)?;
    git(&root, &["rebase", &commit]).map(|_| ())
}

pub fn continue_operation(path: impl AsRef<Path>) -> Result<(), RiftError> {
    let root = repository_root(path.as_ref())?;
    ...
    match state.operation {
        ...
        None => return Err(RiftError::GitFailed("no operation to continue".to_owned())),
    }
    .map(|_| ())
}

pub fn abort_operation(path: impl AsRef<Path>) -> Result<(), RiftError> {
    let root = repository_root(path.as_ref())?;
    match operation_state(&root)?.operation {
        ...
        None => return Err(RiftError::GitFailed("no operation to abort".to_owned())),
    }
    .map(|_| ())
}
...
pub fn mark_resolved(path: impl AsRef<Path>, file: &str) -> Result<(), RiftError> {
    let root = repository_root(path.as_ref())?;
    git(&root, &["add", "--", file]).map(|_| ())
}

fn repository_root(path: &Path) -> Result<PathBuf, RiftError> {
    Ok(PathBuf::from(
...
fn git_path(root: &Path, name: &str) -> Result<PathBuf, RiftError> {
    let output = git(
        root,
...
pub(crate) fn git(directory: &Path, arguments: &[&str]) -> Result<String, RiftError> {
    ...
    if !output.status.success() {
        ...
        return Err(RiftError::GitFailed(message));
    }
