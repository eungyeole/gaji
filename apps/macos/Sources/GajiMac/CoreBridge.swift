...
enum CoreBridge {
    ...
    static func commitFiles(_ path: String, commit: String) throws -> [CoreCommitFileChange] {
        let pointer = path.withCString { pathPointer in
            commit.withCString { gaji_commit_files_json(pathPointer, $0) }
        }
    ...
    static func commitFileDiff(_ path: String, commit: String, file: String) throws -> String {
        let pointer = path.withCString { pathPointer in
            commit.withCString { commitPointer in
                file.withCString { gaji_commit_file_diff_json(pathPointer, commitPointer, $0) }
            }
        }
    ...
    static func hunks(_ path: String, file: String, staged: Bool) throws -> [CoreDiffHunk] {
        let pointer = path.withCString { pathPointer in
            file.withCString { filePointer in
                gaji_file_hunks_json(pathPointer, filePointer, staged)
            }
        }
    ...
    static func fileDiff(_ path: String, file: String, staged: Bool) throws -> CoreFileDiff {
        let pointer = path.withCString { pathPointer in
            file.withCString { filePointer in
                gaji_file_diff_json(pathPointer, filePointer, staged)
            }
        }
    ...
    static func rebasePlan(_ path: String, upstream: String) throws -> [CoreRebaseStep] {
        let pointer = path.withCString { pathPointer in
            upstream.withCString { upstreamPointer in
                gaji_interactive_rebase_plan_json(pathPointer, upstreamPointer)
            }
        }
    ...
    static func blame(_ path: String, file: String) throws -> [CoreBlameLine] {
        let pointer = path.withCString { pathPointer in
            file.withCString { filePointer in gaji_blame_json(pathPointer, filePointer) }
        }
    ...
    static func fileHistory(_ path: String, file: String) throws -> [CoreHistoryEntry] {
        let pointer = path.withCString { pathPointer in
            file.withCString { filePointer in gaji_file_history_json(pathPointer, filePointer) }
        }
        return try decode(pointer, as: [CoreHistoryEntry].self)
    }

    static func worktrees(_ path: String) throws -> [CoreWorktree] {
        try decode(path.withCString(gaji_worktrees_json), as: [CoreWorktree].self)
    }

    static func submodules(_ path: String) throws -> [CoreSubmodule] {
        try decode(path.withCString(gaji_submodules_json), as: [CoreSubmodule].self)
    }

    static func stashes(_ path: String) throws -> [CoreStash] {
        try decode(path.withCString(gaji_stashes_json), as: [CoreStash].self)
    }

    static func stashFiles(_ path: String, index: Int) throws -> [CoreCommitFileChange] {
        try decode(path.withCString { gaji_stash_files_json($0, index) }, as: [CoreCommitFileChange].self)
    }

    static func stashFileDiff(_ path: String, index: Int, file: String) throws -> String {
        let pointer = path.withCString { pathPointer in
            file.withCString { gaji_stash_file_diff_json(pathPointer, index, $0) }
        }
        return try decode(pointer, as: String.self)
    }
    ...
    static func execute(json: String) throws {
        let pointer = json.withCString(gaji_execute_json)
    ...
    private static func consume(_ pointer: UnsafeMutablePointer<CChar>?) throws -> Data {
        ...
        defer { gaji_string_free(pointer) }
